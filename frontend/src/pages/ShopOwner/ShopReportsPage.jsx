...
const ShopReportsPage = () => {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      const data = await shopService.getShopReports({
        startDate,
        endDate,
  ...
  // Build items list string for billing overview
  const getItemsOverview = (orderItems) => {
    return orderItems.map(item => `${item.name || item.product?.name} (x${item.quantity})`).join(', ');
  };

  return (
    <div className="bg-secondary-dark p-6 rounded-lg shadow-lg">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-text-primary">Shop Reports & Analytics</h1>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="bg-primary/20 border border-primary p-6 rounded-lg text-center">
                  <p className="text-sm text-primary mb-2 font-semibold uppercase">Total Paid Orders</p>
                ...
                <div className="bg-green-500/20 border border-green-500 p-6 rounded-lg text-center">
                  <p className="text-sm text-green-400 mb-2 font-semibold uppercase">Total Shop Revenue (₹) [After 30% Cut]</p>
                  <p className="text-3xl font-bold text-text-primary">₹{reportData.summary.totalRevenue.toFixed(2)}</p>
                </div>
              </div>

              <h2 className="text-xl font-semibold text-text-primary mb-4">Detailed Invoices / Bills</h2>
                ...
                <table className="min-w-full print:w-full print:text-xs">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-800/50 text-text-secondary text-sm">
                      <th className="px-5 py-4 text-left font-medium border-b border-gray-200 dark:border-gray-700">Invoice ID</th>
                      <th className="px-5 py-4 text-left font-medium border-b border-gray-200 dark:border-gray-700">Date</th>
                      <th className="px-5 py-4 text-left font-medium border-b border-gray-200 dark:border-gray-700">Customer</th>
                      <th className="px-5 py-4 text-left font-medium border-b border-gray-200 dark:border-gray-700">Items (Qty)</th>
                      <th className="px-5 py-4 text-left font-medium border-b border-gray-200 dark:border-gray-700">Total Items MRP</th>
                      <th className="px-5 py-4 text-left font-medium border-b border-gray-200 dark:border-gray-700">Shop Payout</th>
                    </tr>
                  </thead>
                  <tbody className="text-text-primary text-sm">
                    {reportData.orders.map(order => (
                      <tr key={order._id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-primary/5 dark:hover:bg-primary/10 transition-colors">
                        ...
                        <td className="px-5 py-4 font-medium">{order.user?.name || 'N/A'}</td>
                        <td className="px-5 py-4 text-xs w-1/3 leading-relaxed text-text-secondary">{getItemsOverview(order.orderItems)}</td>
                        <td className="px-5 py-4 text-blue-500 dark:text-blue-400 font-medium">₹{order.pricing?.itemsPrice?.toFixed(2)}</td>
                        <td className="px-5 py-4 font-bold text-green-600 dark:text-green-400">₹{order.pricing?.shopPayout?.toFixed(2)}</td>
                      </tr>
                    ))}
                    {reportData.orders.length === 0 && (
                      <tr><td colSpan="6" className="text-center py-4 text-text-secondary">No invoices found for this period.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
